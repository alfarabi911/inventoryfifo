...
$status = 0;
$nakar = $conn->real_escape_string($_POST['nama']);
...
if ($data_stok >= $qty) {
    ...
    if ($jumlah_brg_keluar > 0) {
        ...
        $sqlM = $conn->query("SELECT * FROM detail_barang_masuk INNER JOIN barang_masuk ON barang_masuk.id_masuk=detail_barang_masuk.id_masuk WHERE kobar='$kobar' AND sisa > 0 ORDER BY tgl_masuk ASC");
        ...
        while ($row_brg_masuk = $sqlM->fetch_assoc()) {
            ...
            $sqldb = $conn->query("INSERT INTO detail_barang_keluar VALUES ('$id_keluar','$kobar','$jml_qty_keluar_sementara','$id_masuk','$status','$nakar','$ruangan')");
