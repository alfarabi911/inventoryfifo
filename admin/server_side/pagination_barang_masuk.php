...
if (isset($_POST['page'])) {
  ...
  $pagConfig = array('baseURL' => 'server_side/pagination_barang_masuk.php', 'totalRows' => $rowCount, 'currentPage' => $start, 'perPage' => $limit, 'contentDiv' => 'posts_content');
  $pagination =  new Pagination($pagConfig);

  //get rows
  $query = $conn->query("SELECT * FROM barang_masuk ORDER BY id_masuk DESC LIMIT $start,$limit");
      ...
      <tbody>
        <thead>
          <tr style="height: 23px;">
            ...
            <th style="width: 246px; height: 23px; text-align: center; background-color: #cff9f7;" colspan="5">ID Masuk</th>
            <th style="width: 246px; height: 23px; text-align: center; background-color: #cff9f7;">Tanggal Masuk</th>
            <th colspan="2" style="width: 246px; height: 23px; text-align: center; background-color: #cff9f7;">Option</th>
          </tr>
        </thead>
        ...
        while ($row = $query->fetch_assoc()) { ?>
          <tr style="height: 23px;">
            ...
            <td style="width: 246px; height: 23px; text-align: center;" colspan="5"><?= $row['id_masuk'] ?></td>
            <td style="width: 246px; height: 23px; text-align: center;"><?= $row['tgl_masuk'] ?></td>
            <td colspan="2" style="width: 246px; height: 23px; text-align: center;">
          ...
          <tr style="height: 23px;">
            ...
            <th style="width: 241px; height: 23px; text-align: center;background-color: #fcf8b5;">Nama Barang</th>
            ...
            <th style="width: 246px; height: 23px; text-align: center;background-color: #fcf8b5;">Rak</th>
            <th style="width: 246px; height: 23px; text-align: center;background-color: #fcf8b5;">Expired</th>
          </tr>
          <?php
          ...
          while ($row2 = $sql2->fetch_assoc()) { ?>
            <tr style="height: 23px;">
              ...
              <td style="width: 246px; height: 23px; text-align: center;">
                <?php $sqlr = $conn->query("SELECT nama_barang FROM barang where kobar='" . $row2['kobar'] . "' ");
              ...
              <td style="width: 246px; height: 23px; text-align: center;">
                <?php $sqlr = $conn->query("SELECT rak FROM barang where kobar='" . $row2['kobar'] . "' ");
                $row_r = $sqlr->fetch_assoc();
                $rak = $row_r['rak']; { ?>
                  <?= $row_r['rak'] ?>
                <?php } ?>
              </td>
              <td style="width: 246px; height: 23px; text-align: center;"><?= $row2['exp'] ?></td>
